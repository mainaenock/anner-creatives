...
export default function ProductDetail({ product, related }: { product: Product; related: Product[] }) {
  ...
  return <main className="flex min-h-screen flex-col bg-[#fbfdff] text-[#17233b]">
    ...
    <section className="mx-auto max-w-[1300px] px-4 pb-14 sm:px-8 lg:px-12"><h2 className="font-serif text-2xl font-bold sm:text-3xl">You may also like</h2><div className="mt-5 grid grid-cols-3 gap-2 sm:gap-5">{related.map((item)=><Link key={item.id} href={`/products/${item.id}`} className="min-w-0 rounded-xl border border-sky-100 bg-white p-1.5 sm:rounded-2xl sm:p-3"><div className="relative aspect-square overflow-hidden rounded-lg" style={{background:item.color}}>{item.image?<Image src={item.image} alt={item.name} fill className="object-cover"/>:<ShoppingBag className="absolute left-1/2 top-1/2 size-10 -translate-x-1/2 -translate-y-1/2 text-white/70"/>}</div><h3 className="mt-2 line-clamp-2 font-serif text-xs font-bold sm:text-lg">{item.name}</h3><p className="mt-1 text-[11px] font-bold sm:text-sm">{money(item.price)}</p></Link>)}</div></section>
    <SiteFooter/>
  </main>;
}
