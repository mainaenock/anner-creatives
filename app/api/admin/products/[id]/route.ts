...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}) {
  try { const {id}=await params; const body=await request.json() as {name?:string;category?:string;description?:string;price?:number;oldPrice?:number|null;active?:boolean;imageKey?:string|null}; const [product]=await getDb().update(products).set(body).where(eq(products.id,Number(id))).returning(); return product?Response.json({product}):Response.json({error:"Product not found."},{status:404}); }
