...
export async function POST(request: Request) {
  ...
  } catch (error) { console.error(JSON.stringify({event:"product_create_failed",error:error instanceof Error?error.message:"unknown"})); return Response.json({error:"Could not save this product."},{status:500}); }
}
