...
export function SiteHeader({ showSearch = false, onSearch, cartCount = 0, onCart }: { showSearch?: boolean; onSearch?: () => void; cartCount?: number; onCart?: () => void }) {
  ...
  return <header className="sticky top-0 z-50 border-b border-sky-100/80 bg-white/95 shadow-[0_1px_12px_rgba(23,35,59,.06)] backdrop-blur-xl">
    <div className="mx-auto flex h-[76px] max-w-[1400px] items-center justify-between px-3 sm:h-[88px] sm:px-8 lg:px-12">
      <Link href="/" className="flex h-[72px] w-[92px] items-center sm:h-[84px] sm:w-[116px]" aria-label="Anner Creatives home"><Image src="/anner-mark-v2.webp" alt="Anner Creatives" width={180} height={180} priority className="h-[70px] w-auto max-w-[92px] object-contain sm:h-[82px] sm:max-w-[116px]"/></Link>
      <nav className="hidden items-center gap-7 text-sm font-semibold md:flex">{links.map((link)=>link.href==="/admin"?<a key={link.href} href={link.href} className="text-slate-600 hover:text-sky-700">{link.label}</a>:<Link key={link.href} href={link.href} className="text-slate-600 hover:text-sky-700">{link.label}</Link>)}</nav>
    ...
    {open&&<nav className="border-t border-sky-100 bg-white px-4 py-3 md:hidden">{links.map((link)=>link.href==="/admin"?<a key={link.href} href={link.href} onClick={()=>setOpen(false)} className="block rounded-xl px-3 py-3 text-sm font-semibold text-slate-700 hover:bg-sky-50 hover:text-sky-700">{link.label}</a>:<Link key={link.href} href={link.href} onClick={()=>setOpen(false)} className="block rounded-xl px-3 py-3 text-sm font-semibold text-slate-700 hover:bg-sky-50 hover:text-sky-700">{link.label}</Link>)}</nav>}
  </header>;
}
